#!/usr/bin/env python3
"""
Vulnerability intelligence monitor using browser-use + Gemini Flash.
Automatically extracts the newest discussions on vulnerabilities, exploits, or 0days.
"""
...
parser = argparse.ArgumentParser(description='Vulnerability discussion extractor using Browser-Use + Gemini')
...
async def extract_latest_article(site_url: str, debug: bool = False) -> dict:
	...
	GOALS
	- Investigate the provided target URL for indicators of vulnerabilities, exploits, or 0day reports.
	- Analyze the website and locate the latest vulnerability or exploit discussions.
	- Analyze each post and gather technical intelligence.

	RULES
	- DO NOT ever go beyond the original domain. Subdomains are valid.
	- When you determine the current page is a Post related to vulnerabilities call the tool `maybe_capture_screenshot`. Do not over-call; one shot per relevant page.
	...
	prompt = (
		f'Navigate to {site_url} and find the section where vulnerabilities are discussed, then find the top 3 most recent posts (based on the creation date) covering vulnerabilities, exploits, proof-of-concept code, or 0day chatter.'
		f"Click on each candidate to open the full post page. Once you load the content, call `maybe_capture_screenshot`, scroll & extract ALL required information: "
		f'1. title: The discussion headline '
		f'2. author: The poster or handle '
		f'3. url: The full URL of the post page '
		f'4. posting_time: The publication date/time as shown on the page '
		f"5. short_summary: A 10-word overview emphasizing the vulnerability or exploit context "
		f'6. long_summary: A 100-word detailed summary of the technical details or claims, if possible '
		...
		f'When done analyzing the top 3 most recent posts, call the done action with success=True and put ALL extracted data in the text field '
	...
	#print(paths)
	#img_b64 = result.screenshots()[-1] 
	#print(img_b64)

	#path_original = "post_monitor_yolo_resources/resources/image"
	#count = 1
	#for image in screenshots:
#		data = base64.b64decode(image)
#		path = f'{path_original}{str(count)}.jpg'
		#with open(path, "wb") as f:
	#		f.write(data)
#		count = count + 1
	#save_images(screenshots)
