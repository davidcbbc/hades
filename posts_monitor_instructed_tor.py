...
async def extract_latest_article(site_url: str, debug: bool = False) -> dict:
	...
	browser_session = BrowserSession(headless=not debug, args=[
        "--proxy-server=socks5://localhost:9050",            # Tor (system Tor: 9050; Tor Browser often 9150)
        "--host-resolver-rules=MAP * ~NOTFOUND , EXCLUDE localhost",  # avoid DNS leaks
        "--proxy-bypass-list=<-loopback>"                    # keep localhost direct
    ])
